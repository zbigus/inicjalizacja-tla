...
\end{equation}
gdzie: $\alpha$ to tempo uczenia, a~$M_{k,t}$ wspó³czynnik, który jest równy 1~dla dopasowanego rozk³adu i~0 dla pozosta³ych rozk³adów. Po dokonaniu aproksymacji wagi s¹ normalizowane. $\frac{1}{\alpha}$ oznacza sta³¹ czasow¹ okreœlaj¹c¹ prêdkoœæ z~jak¹ parametry rozk³adów siê zmieniaj¹.
...
\noindent gdzie: $B(x,y)$ oznacza wartoœæ piksela t³a w~miejscu (x,y), $P(x,y)$ oznacza zbiór wartoœci piksela na pozycji (x,y) we wszystkich ramkach w buforze, $\widetilde{P(x,y)}$ oznacza medianê (wartoœæ œrodkow¹) na pozycji (x,y) we wszystkich ramkach w buforze, $P_{i}(x,y)$ oznacza wartoœæ piksela na pozycji (x,y) w~$i$-tej ramce, gdzie $i \in \{1,2,\dots,n\}$.
