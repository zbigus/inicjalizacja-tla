...
\label{blo:wstep}
Metody przestrzenne, w~przeciwieñstwie do tych operuj¹cych na poziomie pojedynczych pikseli, rozwa¿aj¹ ramkê wideo jako grupê bloków o~okreœlonym rozmiarze oraz wspó³rzêdnych, lokalizuj¹cych jednoznacznie dany blok w~danej ramce. 
Za³o¿enie tej klasy algorytmów polega na przypuszczeniu, i¿ blok który pojawia siê najczêœciej w~danej sekwencji wideo jest najlepszym kandydatem na bycie fragmentem t³a. Bloki o~tych samych wspó³rzêdnych s¹ porównywane miêdzy sob¹ w~kolejnych ramkach sekwencji wideo (np. przy u¿yciu wspó³czynnika korelacji poszczególnych pikseli) w celu utworzenia grupy kandydatów - zbioru bloków, które z~pewnym prawdopodobieñstwem nale¿¹ do t³a i na podstawie którego (z u¿yciem odpowiednich transformat) t³o to zostanie odtworzone. 
Pierwszy etap tego procesu mo¿na zilustrowaæ za pomoc¹ schematu blokowego przedstawionego na rysunku \ref{fig:etap1przestrzenne}.
...
\newpage
\begin{figure}[H]
\includegraphics{etap1przestrzenne.png}
\caption{Schemat blokowy I fazy metod blokowych}
\label{fig:etap1przestrzenne}
\end{figure}

\section{Algorytm metod przestrzennych}
\label{blo:algorytm}
...
Kluczowe dla prawid³owego dzia³ania algorytmu jest dobranie odpowiednich kryteriów podobieñstwa bloków (a raczej wektorów blokowych, gdy¿ na nich wykonywane s¹ obliczenia). Najczêœciej w~tym celu u¿ywa siê wspó³czynnika korelacji oraz wspó³czynnika MAD\footnote{MAD - Mean of Absolute Differences}. S¹ one wyliczane nastêpuj¹co:
\begin{equation}
\label{Tcorr}
...
W celu przeprowadzenia estymacji t³a zosta³o wprowadzone pojêcie superbloku, definiowanego jako klaster o~wymiarach $2 \times 2$ bloki.
\begin{figure}[ht]
\begin{center}
...
Przyk³adowo, dla bloku X~z~rysunku \ref{blokX} mo¿emy wyró¿niæ nastêpuj¹ce superbloki: \{B,C,A,X\}, \{C,D,X,E\}, \{A,X,H,G\} oraz \{X,E,G,F\}.
...
\subsubsection{Estymacja z~wykorzystaniem DCT}
 ...
 \begin{enumerate}
 \item blok X~jest zerowany, natomiast brana jest pod uwagê zawartoœæ s¹siednich bloków. Na superbloku jest przeprowadzana dwuwymiarowa dyskretna transformata kosinusowa, a~jej wspó³czynniki s¹ zapisywane w~macierzy $C$ o~wymiarach $M \times M$. Wspó³czynnik DC macierzy C~(o wspó³rzêdnych (0,0)) jest ustawiany na 0, przez co pod uwagê zostanie wziête tylko przestrzenne zró¿nicowanie wartoœci poszczególnych pikseli, 
 ...
 \end{enumerate}
  ...
  Blok o~najni¿szej wartoœci funkcji kosztu (czyli taki, który najlepiej redukuje sumê wysokich czêstotliwoœci w~macierzach $C$~i~$D$~) zostaje dodany jako najbardziej wiarygodna estymacja t³a.
   ...
   Metoda zosta³a przedstawiona po raz pierwszy w~pracy \cite{Baltieri2010}. Ka¿da ramka jest w niej dzielona na bloki o~rozmiarze $16 \times 16$ pikseli. Do przeprowadzenia trzeciego etapu tej metody jest wykorzystywana dyskretna transformata Hadamarda, bêd¹ca generalizacj¹ transformaty Fouriera. Opiera siê ona na macierzach Hadamarda $H$, definiowanych rekursywnie w~nastêpuj¹cy sposób:
   \begin{equation}
    H_1=\left[1\right] \\
    \end{equation}
    \begin{equation}
   ...
   Transformatê $F$~bloku $X$ o~wymiarach $2N \times 2N$ wyra¿a siê jako:
   \begin{equation}
   F=MXM
   \end{equation}
   gdzie: $M$~to macierz Hadamarda rzêdu $2N$. Bardzo u¿yteczn¹ w³asnoœci¹ tej transformaty jest mo¿liwoœæ rozbicia jej na sumê transformat rzêdu ni¿szego. Przyk³adowo, po rozbiciu bloku $X$~na podbloki $A$,$B$,$C$,$D$ o~wymiarach $N\times N$  transformatê mo¿na obliczyæ ze wzoru:
   \begin{equation}F=
   \begin{bmatrix}
   H& H\\
   H & -H
