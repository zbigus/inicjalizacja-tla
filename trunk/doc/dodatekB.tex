...
Przyk³adowe wywo³anie programu u¿ywaj¹cego do obliczeñ transformaty Hadamarda dla sekwencji clip\_01.mpg z parametrami $T_{corr}=0.8$ oraz $T_{MAD}=20$: \textbf{robust.exe clip\_01.mpg 200 1 16 0.8 20}.
Program sk³ada siê z nastêpuj¹cych klas:

\subsection{blok}
Klasa blok przechowuje w kontenerze typu vector zawartoœæ pojedynczego bloku. Posiada ona równie¿ zmienn¹ \texttt{weight}, reprezentuj¹c¹ wagê bloku oraz \texttt{size}, okreœlaj¹c¹ jego rozmiar. Jej najwa¿niejsze metody to:
\begin{description}
...
Klasa background przechowuje odtworzone fragmenty t³a w kontenerze typu vector. Zawiera równie¿ tablicê \texttt{int**filltable}, okreœlaj¹c¹, czy dana lokalizacja blokowa posiada ju¿ odtworzone t³o - jeœli tak, to w~tablicy w miejscu o wspó³rzêdnych x i y bêdzie znajdowaæ siê wartoœæ 1 oraz licznik \texttt{int nrfilled} okreœlaj¹cy, ile fragmentów t³a zosta³o ju¿ odtworzonych. 
...
\end{description}
\subsection{robust}
Robust jest to g³ówna klasa, wykonuj¹ca obliczenia na sekwencjach wideo i zwracaj¹ca obrazy wynikowe t³a. Po uruchomieniu, gdy zostanie wywo³ana dla metod blokowych, tworzy ona pojedyncze obiekty klas grid (do przechowywania grupy kandydatów) i background (do przechowywania odtworzonego t³a), a nastêpnie przetwarza ka¿d¹ kolejn¹ ramkê zgodnie z algorytmem opisanym w sekcji \ref{blo:algorytm}. W~przypadku wywo³ania jej dla metody MOG, u¿ywa ona wbudowanej w OpenCV klasy BackgroundSubtractorMOG2.
...
\item œcie¿ka do analizowanej sekwencji wideo,
\item wielkoœæ bufora ramek,
\item u¿ywana metoda: 0 - aproksymacja, 1 - œrednia, 2 - mediana,
\item wartoœæ parametru $\alpha$ (jedynie w przypadku metody aproksymacji).
\end{enumerate}
Przyk³adowe wywo³anie programu u¿ywaj¹cego do obliczeñ metody aproksymacji œredniej z bufora dla sekwencji clip\_02.mpg z parametrem $\alpha=0.95$: \textbf{simple\_pixel\_methods.exe clip\_01.mpg 200 0 0.95}.
Program sk³ada siê z nastêpuj¹cych klas:
\subsection{alpha}
...
\item[void process(const cv::Mat \&img\_input, cv::Mat \&img\_output)] - funkcja odpowiedzialna za obliczanie t³a metod¹ aproksymacji œredniej. Pierwszym przyjmowanym parametrem jest referencja do obiektu zawieraj¹cego pojedyncz¹ ramkê obrazu, natomiast drugim jest referencja do obiektu przechowuj¹cego obliczone t³o.
\end{description}

\subsection{cachedMethods}
Klasa cachedMethods jest odpowiedzialna za obliczanie metod: œredniej z bufora oraz mediany. Jej najwa¿niejsze funkcje to:
\begin{description}
